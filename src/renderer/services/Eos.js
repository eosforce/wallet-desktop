...
import { toAsset, toBigNumber, getTimeStamp, isZero, calcTotalAmount, clacReward, handleApiError, clacAverage } from '@/utils/util'
...
export const getRewardsAndBpsTable = httpEndpoint => async (votesTable, accountName) => {
  ...
  for (const [index, bpRow] of bpsTable.entries()) {
    ...
    const average = clacAverage(total_voteage, total_staked, voteage_update_time)

    bpRow.average = average
    if (vote) {
      const { bpname, staked, stake_time, unstaking } = vote
      const reward = toAsset(clacReward(staked, stake_time, total_voteage, rewards_pool, total_staked))
