<template>
  ...
  <!-- <confirm-modal :showConfirm="true" :show="true" :submitting="submitting" @confirm="submit()" @close="close"> -->
    <div>
      <div class="graphic">
      ...
      <div class="row">
        ...
        <div class="row__content">{{ symblo_change( unstaking ) | formatNumber({p: 4, showSymbol: true, symbol: wallet_symbol})  }}</div>
      </div>
...
export default {
  ...
  computed: {
    ...
    unstakeHeight() {
      ...
      if (bp) {
        if (bp.ramvote) {
          return bp.ramvote.unstake_height;
        } else {
          return null;
        }
      } else {
  ...
  methods: {
    ...
    ...mapActions({
      ...
      unfreeze4ram: Actions.UNFREEZE4RAM,
      UNFREEZECPUNET: Actions.UNFREEZECPUNET
    }),
  },
  created() {
