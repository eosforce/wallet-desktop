    ...
    <div class="box" v-if="!on_load_info">
      <div class="publickey">
        <span class="pk" style="min-width: 25%;width:auto;margin-right: 20px;">
          ...
          {{$route.params.accountName}}<span v-if="!on_load_info" v-for="item in permissions" class="permission_tag" v-bind:class="{'permission_tag_not_have': !item.is_have}">{{ item.name }}</span>
          </span>
        </span>
        ...
        <span>
          ...
          <router-link v-if="has_owner" style="width: 110px;background: #408ee1;border-radius: 17.5px;color: #fff;border: none;" class="button is-small is-link" :to="{name: 'accountTransfer'}">
            {{$t('账户转让')}}
          </router-link>
